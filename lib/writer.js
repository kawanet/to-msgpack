...
const {isMsg} = require("msg-interface");
...
exports.initWriter = (options) => {
  ...
  function writeObject(writable, value) {
    ...
    if (isMsg(value)) {
      return writeMsgpack(writable, value);
    }
...
function writeMsgpack(writable, value) {
  const length = +value.msgpackLength;

  if (length < 1024) {
    // shorter
    return writable.reserve(length, writeMsgpackTo, value);
  } else {
    // longer or valid byteLength not given
    const buffer = value.toMsgpack();
    return writable.insertBuffer(buffer);
  }
}

function writeMsgpackTo(buffer, offset, value) {
  value.writeMsgpackTo(buffer, offset);
}
