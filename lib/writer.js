...
const {Msg} = require("msg-interface");
...
function writeMsg(writable, value) {
  // In case byteLength property given
  const length = +value.byteLength;
  if (length) {
    return writable.reserve(length, msgWriteTo, value);
  }

  // In case byteLength property NOT given
...
function msgWriteTo(buffer, offset, value) {
  value.writeTo(buffer, offset);
}
